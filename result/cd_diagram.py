...
def wilcoxon_holm(alpha=0.05, df_perf=None):
    ...
    friedman_p_value = friedmanchisquare(*(
        np.array(df_perf.loc[df_perf['classifier_name'] == c]['f1'])
    ...
    for i in range(m - 1):
        ...
        perf_1 = np.array(df_perf.loc[df_perf['classifier_name'] == classifier_1]['f1']
                          , dtype=np.float64)
    ...
    # get the rank data
    rank_data = np.array(sorted_df_perf['f1']).reshape(m, max_nb_datasets)
...
draw_cd_diagram(df_perf=df_perf, title='f1', labels=True)
